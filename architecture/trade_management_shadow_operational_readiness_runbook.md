...
5. Submit the local-paper BUY using the deterministic idempotency key returned by
   `paper_thesis_entry_idempotency_key(draft)`.
6. Activate the authoritative TradeThesis only from the resulting Journaled fill. The fill must retain
