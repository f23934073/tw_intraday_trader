...
_BUILTIN_BINDINGS = {
    ...
    "backtest.opening_range_breakout_entry_v1",
    "backtest.ema_crossover_entry_v1",
    "backtest.rsi_bollinger_reversion_entry_v0",
    "backtest.atr_stop_exit_v1",
    "backtest.time_stop_exit_v1",
    "backtest.sma_20_60_golden_cross_entry_v1",
    "backtest.sma_20_60_death_cross_exit_v1",
    "candidate.gap_up",
    "candidate.high_volume",
    "candidate.relative_volume",
    "candidate.premarket_gap_up",
    "premarket.taifex_overnight_context_v0",
    "scoring.gap_score",
    "scoring.above_vwap",
...
def builtin_definitions(registry: Any) -> tuple[StrategyDefinition, ...]:
    ...
    definitions.extend(
        [
            ...
            _catalog_definition(
                strategy_id="taifex_overnight_context_v0",
                display_name_zh_tw="台指期夜盤市場情境",
                version="v0",
                role=StrategyRole.SIGNAL,
                session_phase=SessionPhase.PRE_MARKET,
                status=StrategyStatus.EXPERIMENTAL,
                description_zh_tw="顯示臺股期貨近月夜盤 signed metrics 與資料完整性；只供觀察，不改候選、評分、風控或委託。",
                execution_binding="premarket.taifex_overnight_context_v0",
                required_capabilities=("TAIFEX_TX_KBAR", "TAIFEX_CALENDAR"),
                parameters={
                    "schema_version": "taifex_night_context_v0",
                    "readiness_predicate_version": "taifex_night_ready_v0",
                    "query_delay_minutes": 5,
                    "observation_only": True,
                    "affects_decisions": False,
                },
                tags=("盤前", "台指期", "市場情境", "實驗中"),
                code_identity="taifex-overnight-context-v0",
            ),
            _catalog_definition(
